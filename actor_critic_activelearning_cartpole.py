...
class PolicyEstimator:
    ...
    def create_actor(self):
        ...
        x = tf.contrib.layers.fully_connected(
            ...
            weights_initializer=tf.uniform_unit_scaling_initializer(-0.01, 0.01))
    ...
    for i_episode in range(num_episodes):
        ...
        for t in itertools.count():
            ...
            if(step > 1 and len(subepisode) > 0):
                ...
                #    subepisode[idx][0], np.reshape(td_error[idx], (-1, 1)), np.reshape(
...
policy_estimator = PolicyEstimator(4, 2, 0.01, 0.001)
value_estimator = ValueEstimator(
    4, 2, 0.1, 0.001, policy_estimator.get_num_trainable_vars())
steps = [6, 3, 1, 1]
...
with open("ac_active_cartpole.pickle", 'wb') as handle:
