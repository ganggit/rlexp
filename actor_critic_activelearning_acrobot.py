...
class PolicyEstimator:
    ...
    def create_actor(self):
        ...
        x = tf.contrib.layers.fully_connected(
            ...
            weights_initializer=tf.uniform_unit_scaling_initializer(-0.1, 0.1))
...
def zoomUpdate(estimator_value, estimator_policy, subepisode, discount_factor, mix):
    ...
    for i in range(num):
        ...
        value_next = estimator_value.predict_target(trans[3])
        if trans[4]:
            value_next = 0
    ...
    for i_episode in range(num_episodes):
        ...
        for t in itertools.count():
            ...
            if(step > 1 and len(subepisode) > 0):
                ...
                # estimator_policy.update(subepisode[idx][0], np.reshape(td_error[idx], (-1, 1)), np.reshape(
...
policy_estimator = PolicyEstimator(6, 3, 0.01, 0.001)
value_estimator = ValueEstimator(
    6, 3, 0.1, 0.001, policy_estimator.get_num_trainable_vars())
steps = [6, 6, 3, 1]
...
with open("ac_activelearning_acrobot.pickle", 'wb') as handle:
